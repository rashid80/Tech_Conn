...
namespace Tech_Conn
{
    class WordWriter
    {
        ...
        public void Save(string fileName, Word.WdSaveFormat fileFormat)
        {
            if (wordDocument != null)
            {
                wordDocument.SaveAs2(fileName, fileFormat);
            }
        }
        ...
        public void AppendIn(string fileName)
        {
            var sel = wordDocument.Application.Selection;
            sel.EndKey(6); //курсор в конец документа
            sel.InsertBreak(2); //Вставляем разрыв разделов (2), для разрыва страницы используем (7)
            sel.InsertFile(fileName); //Вставляем второй документ
        }
